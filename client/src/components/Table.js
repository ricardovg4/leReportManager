...
const Table = (props) => {
  ...
  const updateRows = async (query, resetPaginationOnUpdate = true, setRowsDataToNull) => {
    let response;
    // Loading state
    try {
  ...
  const currentRowsData = (rowsData) => {
    ...
    const lastPage = rowsData.length >= start && rowsData.length <= last ? true : false;
      ...
      <div className={`modal ${editModal}`}>
        ...
        <div className="modal-card">
          <header className="modal-card-head">
            ...
            <button className="delete" onClick={() => setEditModal('')}></button>
          </header>
  ...
  return (
    <div>
      ...
      <hr />
      <div className="table-container" style={{ height: '60vh', overflowY: 'auto' }}>
